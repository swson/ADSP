...
# matrix_names and dataset_range
matrix_names = ["494_bus", "662_bus", "1138_bus", "bcsstk01", "bcsstk19", "bcsstk20", "bfwb62", "bfwb398","bfwb782" ]    
dataset_range = range(1, 6)    # 494_bus_1 ~ 494_bus_5
#error_metric = ["point_gauss"]

# --------------------------------------------------------
# 1. Hardware Events (그대로 유지)
# --------------------------------------------------------
event_name_arr = [
...
def perform_run(batch_id, num_runs, max_counter, event_name_arr,
    ...
    base_name = f"{output_dir}/{matrix_name}_point_guass_0.00_0.{batch_id + 1}.mtx_1"
    ...
    for run_number in range(num_runs):
        ...
        for total_groups_created in range(1, num_loops_needed + 1):
            print(f"[{matrix_name}] [Batch {batch_id}] CLEAN Run {run_number}, Group {total_groups_created}")
            ...
            perf_cmd = (
                ...
                f"./crs_matmult --matrix {matrix_path} >> /dev/null"
            )
...
# 3. ERROR RUN
# --------------------------------------------------------
def perform_run_error(matrix_file, batch_id, num_runs, max_counter,
                      event_name_arr, output_dir, matrix_name):
    ...
    for run_number in range(num_runs):
        ...
        for total_groups_created in range(1, num_loops_needed + 1):
            print(f"[{matrix_name}] [Batch {batch_id}] ERROR Run {run_number}, Group {total_groups_created}")
...
# --------------------------------------------------------
# 4. MAIN LOOP (Matrix + Dataset 자동 반복)
# --------------------------------------------------------
for matrix_name in matrix_names:
    for dataset_id in dataset_range:

        matrix_path = f"input_point_gauss_avg/{matrix_name}_{dataset_id}/crs/non_zero/{matrix_name}.mtx.crs"
        input_dir   = f"input_point_gauss_avg/{matrix_name}_{dataset_id}/crs/non_zero"
        output_dir  = f"output_point_gauss_original_avg/{matrix_name}_{dataset_id}"
        ...
        input_files = sorted([
            ...
            if f.startswith(f"{matrix_name}_point_gauss_") and f.endswith(".crs")
        ])

        print("\n====================================================")
        print(f"Matrix     : {matrix_name}")
        print(f"Dataset ID : {dataset_id}")
        print(f"#ErrorFiles: {len(input_files)}")
        print("====================================================")

        # CLEAN (baseline)
        for batch_id in range(100):
            perform_run(batch_id, 1, max_counter,
                        event_name_arr, matrix_path,
                        output_dir, matrix_name)

        # ERROR
        for matrix_file in input_files:
            perform_run_error(matrix_file, 0, 1,
                              max_counter, event_name_arr,
                              output_dir, matrix_name)
