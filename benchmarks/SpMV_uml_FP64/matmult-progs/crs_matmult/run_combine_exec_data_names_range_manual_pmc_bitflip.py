...
def perform_run(batch_id, num_runs, max_counter, event_name_arr,
    ...
    base_name = f"{output_dir}/{matrix_name}_mantissaFlip1_0.00_0.{batch_id + 1}.mtx_1"
...
for matrix_name in matrix_names:
    for dataset_id in dataset_range:
        ...
        matrix_path = f"data_bitflip_LSB/{matrix_name}_{dataset_id}/crs/non_zero/{matrix_name}.mtx.crs"
        input_dir   = f"data_bitflip_LSB/{matrix_name}_{dataset_id}/crs/non_zero"
        output_dir  = f"output_bitflip_LSB/{matrix_name}_{dataset_id}"
        ...
        input_files = sorted([
            ...
            if f.startswith(f"{matrix_name}_mantissaFlip1_") and f.endswith(".crs")
        ])
