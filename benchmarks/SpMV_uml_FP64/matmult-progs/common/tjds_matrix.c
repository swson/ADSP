	...
	while(fscanf(stream, "%ms", &read_data) != EOF) {
		switch(state) {
			...
			case MATRIX_VALUE_LIST_LEN:
				...
				ret->value_list = (double*)malloc(sizeof(double) * ret->value_list_len);
				if(!(ret->value_list)) {
					goto err_no_value;
				}
				ret->row_indexes =
						(size_t*)malloc(sizeof(size_t) * ret->value_list_len);
				if(!(ret->row_indexes)) {
			...
			case PERM_ARRAY_LEN:
				...
				ret->perm_array =
						(size_t*)malloc(t * sizeof(*(ret->perm_array)));
				if(!(ret->perm_array)) {
					goto err_no_jagged_diagonal;
				}
...
const double* get_tjds_matrix_val_array(const struct tjds_matrix* m) {
	return m->value_list;
}
...
int main(void) {
	...
	m = load_tjds_matrix(stdin);
	assert(m != NULL);
	dump_tjds_matrix(stdout, m);
	m = destroy_tjds_matrix(m);
