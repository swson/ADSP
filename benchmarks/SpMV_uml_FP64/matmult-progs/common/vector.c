...
struct vector* make_col_ones(size_t rows) {
	...
	/* allocate data array */
	if(!(ret->data = (double*)malloc(rows * sizeof(*(ret->data))))) {
		free(ret);
		return (struct vector*)NULL;
	}
...
double get_vector_value(const struct vector* v, size_t offset) {
...
const double* get_vector_data(const struct vector* v) {
	return v->data;
}

double* get_vector_data_rw(const struct vector* v) {
	return v->data;
}
...
struct vector* scramble_and_swap_vector(struct vector* restrict dest,
		struct vector* restrict src, const size_t* restrict p) {
...
int scramble_vector(struct vector* v, const size_t* restrict p) {
	...
	if(!(pv_data = (double*)malloc(sizeof(double) * v->data_len))) {
		return -1;
	}
