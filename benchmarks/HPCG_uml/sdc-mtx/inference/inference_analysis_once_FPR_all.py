...
class Inference:
    ...
    def read_data(self, file_name):
        ...
        df = pd.read_csv(path)  # ✅ index_col 제거

        # 1) error_rate를 숫자로 강제 변환 (문자면 NaN)
        er_num = pd.to_numeric(df["error_rate"], errors="coerce")

        # 2) 만약 error_rate에 b0-0 같은 값이 섞여 NaN이 많으면,
        #    injection_rate 컬럼이 실제 error_rate일 가능성이 높음
        if er_num.isna().any():
            # error_rate 컬럼의 문자열은 flip_pos로 보고,
            # injection_rate를 실제 error_rate로 사용
            er_num = pd.to_numeric(df["injection_rate"], errors="coerce")
            inj_num = np.nan  # 이 케이스에서는 injection_rate 자체가 없다고 보고 NaN 처리
        else:
            inj_num = pd.to_numeric(df["injection_rate"], errors="coerce")

        # ✅ label: error_rate != 0  → 0: Normal, 1: Error
        y = (er_num.fillna(0.0) != 0.0).astype(int).to_numpy()

        # ✅ feature: 메타 컬럼 제외하고 나머지 전부 카운터로 사용
        meta_cols = [c for c in ["matrix", "flip_pos", "error_rate", "injection_rate"] if c in df.columns]
        X_df = df.drop(columns=meta_cols)

        # 숫자 아닌 값이 혹시 있으면 NaN으로 바꾸고 0으로 채움
        X = X_df.apply(pd.to_numeric, errors="coerce").fillna(0.0).to_numpy(dtype=np.float64)
...
if __name__ == "__main__":
    ...
    data_root = "output_all/point_gauss_keep_decimal_point"
    out_root = "inf_all/inf_point_gauss_keep_decimal_point"
