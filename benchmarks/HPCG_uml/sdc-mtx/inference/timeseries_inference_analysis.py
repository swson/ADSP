...
Input:
    inference/data/timeseries_merged_rows.csv
...
This script:
1. Loads row-level PMU data generated by timeseries_dataset_builder.py
2. Uses each timestamp row as one ML sample
3. Removes metadata columns from ML features
4. Splits train/test by test_id to avoid data leakage
5. Uses StratifiedGroupKFold for cross-validation
6. Trains multiple baseline classifiers
7. Evaluates accuracy, precision, recall, F1, ROC-AUC
8. Saves comparison results, best model, predictions, confusion matrix, and feature importance
"""
...
class TimeSeriesInference:
    def __init__(
        self,
        data_path: str = "inference/data/timeseries_merged_rows.csv",
...
if __name__ == "__main__":
    analysis = TimeSeriesInference(
        data_path="inference/data/timeseries_merged_rows.csv",
