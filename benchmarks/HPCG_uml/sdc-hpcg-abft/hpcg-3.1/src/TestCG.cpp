...
/*!
  ...
  @see CG()
 */
int TestCG(SparseMatrix & A, SparseMatrix & A_pure, CGData & data, Vector & b, Vector & x, TestCGData & testcg_data) {
  ...
  for (int k=0; k<2; ++k) { // This loop tests both unpreconditioned and preconditioned runs
    ...
    for (int i=0; i< numberOfCgCalls; ++i) {
      ...
      int ierr = CG(A, A_pure,  data, b, x, maxIters, tolerance, niters, normr, normr0, &times[0], k==1);
