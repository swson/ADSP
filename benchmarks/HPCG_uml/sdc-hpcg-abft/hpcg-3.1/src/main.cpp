...
int main(int argc, char * argv[]) {
  ...
  char *test_id_str = getenv("HPCG_TEST_ID");
  if (test_id_str)
      test_id = atoi(test_id_str);
  ...
  SparseMatrix A, A_pure;
  InitializeSparseMatrix(A, geom);
  InitializeSparseMatrix(A_pure, geom);

  Vector b, x, xexact;
  GenerateProblem(A, A_pure, &b, &x, &xexact);

  // === A copy of A as A_pure ===
  //SparseMatrix A_pure;
  //  CopyMatrix(A, A_pure);
  ...
  for (int i=0; i< numberOfCalls; ++i) {
    ...
    sprintf(buf, "spv_ref_core_%d_%.2f_%.2f_%d.txt", i, err_rate, inj_rate, test_id);
    TRACE_ALL_CORE(buf, &p, ComputeSPMV_ref, A, x_overlap, b_computed);
    sprintf(buf, "mg_ref_core_%d_%.2f_%.2f_%d.txt", i, err_rate, inj_rate, test_id);
    TRACE_ALL_CORE(buf, &p, ComputeMG_ref, A, b_computed, x_overlap); // b_computed = Minv*y_overlap
#else
    sprintf(buf, "spv_ref_uncore_%d_%.2f_%.2f_%d.txt", i, err_rate, inj_rate, test_id);
    TRACE_ALL_UNCORE(buf, &p, ComputeSPMV_ref, A, x_overlap, b_computed);
    sprintf(buf, "mg_ref_uncore_%d_%.2f_%.2f_%d.txt", i, err_rate, inj_rate, test_id);
  ...
  for (int i=0; i< numberOfCalls; ++i) {
    ZeroVector(x);
#ifndef HPCG_TRACE_UNCORE
    sprintf(buf, "cg_ref_core_%d_%.2f_%.2f_%d.txt", i, err_rate, inj_rate, test_id);
    TRACE_ALL_CORE(buf, &p, CG_ref, A, data, b, x, refMaxIters, tolerance, niters, normr, normr0, &ref_times[0], true);
#else
    sprintf(buf, "cg_ref_uncore_%d_%.2f_%.2f_%d.txt", i, err_rate, inj_rate, test_id);
    TRACE_ALL_CORE(buf, &p, CG_ref, A, data, b, x, refMaxIters, tolerance, niters, normr, normr0, &ref_times[0], true);
#endif
  ...
  TestCG(A, A_pure, data, b, x, testcg_data);
...
#ifndef HPCG_TRACE_UNCORE
    sprintf(buf, "cg_core_%d_%.2f_%.2f_%d.txt", i, err_rate, inj_rate, test_id);
    TRACE_ALL_CORE(buf, &p, CG, A, A_pure, data, b, x, optMaxIters, refTolerance, niters, normr, normr0, &opt_times[0], true);
#else
    sprintf(buf, "cg_uncore_%d_%.2f_%.2f_%d.txt", i, err_rate, inj_rate, test_id);
    TRACE_ALL_UNCORE(buf, &p, CG, A, A_pure, data, b, x, optMaxIters, refTolerance, niters, normr, normr0, &opt_times[0], true);
#endif
...
#ifndef HPCG_TRACE_UNCORE
    sprintf(buf, "cg_timed_core_%d_%.2f_%.2f_%d.txt", i, err_rate, inj_rate, test_id);
    TRACE_ALL_CORE(buf, &p, CG, A, A_pure, data, b, x, optMaxIters, optTolerance, niters, normr, normr0, &times[0], true);
#else
    sprintf(buf, "cg_timed_uncore_%d_%.2f_%.2f_%d.txt", i, err_rate, inj_rate, test_id);
    TRACE_ALL_UNCORE(buf, &p, CG, A, A_pure, data, b, x, optMaxIters, optTolerance, niters, normr, normr0, &times[0], true);
#endif
