...
int main(int argc, char * argv[]) {
  ...
  bool skipCheckProblem = (getenv("HPCG_MATRIX_PATH") != nullptr);
  ...
  Vector * curxexact = &xexact;
    ...
    } else if (curLevelMatrix->geom->rank == 0 && level == 0) {
      std::cout << "[INFO] Skipping CheckProblem at level " << level << " (MTX-based matrix)" << std::endl;
    }

    curLevelMatrix = curLevelMatrix->Ac; // Next coarse grid
    curb = nullptr;
    curx = nullptr;
    curxexact = nullptr;
  }
...
// Get the absolute worst time across all MPI ranks (time in CG can be different)
  ...
  testnorms_data.values.resize(numberOfCgSets);
