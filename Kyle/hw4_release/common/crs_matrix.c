...
struct crs_matrix* load_crs_matrix(FILE* stream) {
	...
	if(!(ret = (struct crs_matrix*)malloc(sizeof(*ret)))) {
	...
	while(fscanf(stream, "%ms", &read_data) != EOF) {
		switch(state) {
			...
			case VALUE_LIST_LEN:
				...
				ret->a =
					(long double*)malloc(sizeof(long double) * ret->data_len);
				if(!(ret->a)) {
					goto err_no_a;
				}
				ret->ja = (size_t*)malloc(sizeof(size_t) * (ret->data_len));
				if(!(ret->ja)) {
					goto err_no_ja;
				}
				state = ROW_LIST_LEN;
			...
			case ROW_LIST_LEN:
				...
				ret->ia =
					(size_t*)malloc(sizeof(size_t) * ret->rows);
				if(!(ret->ia)) {
					goto err_no_ia;
				}
				state = DATA_NUMBER;
...
int main(void) {
	struct crs_matrix* m = load_crs_matrix(stdin);
	assert(m != NULL);
