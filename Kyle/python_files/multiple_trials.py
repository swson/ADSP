...
path_to_images = os.path.join(project_home,"data",matrix_name,trial_name,run_name,"images")
...
def run_matmult_process_perf_stat(matrix_file_names_list, event_name_string,trial_path):
	...
	for i in range( len(matrix_file_names_list) ):		
		...
		subprocess.run( ["mv -f "+ matrix_file_names_list[i] + ".csv" + " " + trial_path ] , shell = True)
...
def get_data_to_plot(data_path):		# this will create the data structure that I will use to plot the graph
...
def plot_data(data,file_name_labels_list,division_size,event_name_arr,fig_output_path,trial_name):

	os.chdir(fig_output_path)
	...
	plt.title( event_file[:-4] + trial_name )
	...
	plt.savefig(trial_name + '.png', dpi = 800 )
...
for run_number in range(num_trials):
	...
	subprocess.run( ["mkdir " + trial_name] , shell = True)
	
	os.chdir(trial_path)

	run_matmult_process_perf_stat(a, event_name_string,trial_path)


	data,file_name_labels_list=get_data_to_plot(trial_path)

	plot_data(data,file_name_labels_list,10,temp,path_to_images,trial_name)
